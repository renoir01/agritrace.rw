...
class FirestoreService {
  ...
  Future<List<FarmerModel>> getAllFarmersOnce() async {
    try {
      final snapshot = await _firestore
  ...
  Future<FarmerModel?> getFarmerByUserId(String userId) async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      return FarmerModel.fromFirestore(querySnapshot.docs.first);
    }
    return null;
  }
  ...
  Future<String> createConsumer(ConsumerModel consumer) async {
    final docRef = await _firestore
  ...
  Future<ConsumerModel?> getConsumer(String id) async {
    final doc = await _firestore
        .collection(AppConstants.consumersCollection)
        .doc(id)
        .get();
    ...
    if (doc.exists) {
      return ConsumerModel.fromFirestore(doc);
    }
    return null;
  }

  Future<ConsumerModel?> getConsumerByUserId(String userId) async {
    final snapshot = await _firestore
        .collection(AppConstants.consumersCollection)
        .where('userId', isEqualTo: userId)
        .limit(1)
    ...
    if (snapshot.docs.isNotEmpty) {
      return ConsumerModel.fromFirestore(snapshot.docs.first);
    }
    return null;
  }

  Stream<List<ConsumerModel>> getAllConsumers({String? consumerType}) {
    Query query = _firestore
        .collection(AppConstants.consumersCollection)
        .where('isActive', isEqualTo: true);

    if (consumerType != null) {
      query = query.where('consumerType', isEqualTo: consumerType);
    }

    return query.snapshots()
        .map((snapshot) => snapshot.docs
            .map((doc) => ConsumerModel.fromFirestore(doc))
            .toList());
  }

  // Get institutional consumers only (schools, hospitals, restaurants)
  Stream<List<ConsumerModel>> getInstitutionalConsumers() {
    return _firestore
        .collection(AppConstants.consumersCollection)
        .where('isActive', isEqualTo: true)
        .where('consumerType', whereIn: [
          AppConstants.schoolConsumerType,
          AppConstants.hospitalConsumerType,
          AppConstants.restaurantConsumerType,
        ])
        .snapshots()
        .map((snapshot) => snapshot.docs
            .map((doc) => ConsumerModel.fromFirestore(doc))
  ...
  // Get individual consumers only
  Stream<List<ConsumerModel>> getIndividualConsumers() {
    return _firestore
        .collection(AppConstants.consumersCollection)
        .where('isActive', isEqualTo: true)
        .where('consumerType', isEqualTo: AppConstants.individualConsumerType)
        .snapshots()
        .map((snapshot) => snapshot.docs
            .map((doc) => ConsumerModel.fromFirestore(doc))
            .toList());
    ...
    try {
      ...
      if (transactionsSnapshot.docs.isNotEmpty) {
        ...
        if (farmerDoc.exists) {
          ...
          chain.add({
            'actorType': 'farmer',
            'actorName': farmerData['farmerName'] ?? farmerData['cooperativeName'] ?? 'Unknown Farmer',
  ...
  Future<Map<String, dynamic>?> getTraceabilityByOrder(String orderId) async {
    try {
      ...
      final sellerDoc = await _firestore
          .collection(orderData['sellerType'] == 'aggregator' ? AppConstants.aggregatorsCollection : AppConstants.farmersCollection)
          .doc(orderData['sellerId'])
          .get();

      final buyerDoc = await _firestore
          .collection(orderData['buyerType'] == 'aggregator' ? AppConstants.aggregatorsCollection : AppConstants.consumersCollection)
      ...
      if (sellerDoc.exists) {
        ...
        chain.add({
          ...
          'actorName': sellerData['businessName'] ?? sellerData['farmerName'] ?? sellerData['cooperativeName'] ?? 'Unknown',
      ...
      if (buyerDoc.exists) {
        ...
        chain.add({
          'actorType': orderData['buyerType'] == 'aggregator' ? 'aggregator' : 'consumer',
          'actorName': buyerData['businessName'] ?? buyerData['organizationName'] ?? buyerData['institutionName'] ?? buyerData['fullName'] ?? 'Unknown',
          'date': DateTime.now().toString().split('T')[0],
  ...
  Future<List<UserModel>> getAllUsers() async {
    try {
      final collections = [
        ...
        AppConstants.farmersCollection,
        AppConstants.aggregatorsCollection,
        AppConstants.consumersCollection,
      ];
  ...
  String _getUserTypeFromCollection(String collection) {
    switch (collection) {
      ...
      case AppConstants.farmersCollection:
        return AppConstants.farmerType;
      case AppConstants.aggregatorsCollection:
        return AppConstants.aggregatorType;
      case AppConstants.consumersCollection:
        return AppConstants.consumerType;
      default:
        return 'unknown';
    }
  }
