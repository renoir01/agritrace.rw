...
class PDFService {
  ...
  Future<void> generateTraceabilityCertificate(OrderModel order) async {
    ...
    if (order.orderType == 'aggregator_to_farmer') {
      farmer = await FirestoreService().getFarmerByUserId(order.sellerId);
      aggregator = await FirestoreService().getAggregatorByUserId(order.buyerId);
    } else if (order.orderType == 'consumer_to_aggregator') {
      aggregator = await FirestoreService().getAggregatorByUserId(order.sellerId);
      consumer = await FirestoreService().getConsumerByUserId(order.buyerId);
    }
        ...
        build: (context) => [
          ...
          _buildSection(
            ...
            [
              ...
              if (consumer != null) ...[
                _buildActorInfo(
                  consumer.consumerType == 'individual' ? 'Consumer' : 'Institutional Buyer',
                  consumer.organizationName ?? consumer.fullName ?? 'Unknown',
                  consumer.location?.district ?? 'N/A',
                  consumer.numberOfBeneficiaries != null ? '${consumer.numberOfBeneficiaries} beneficiaries' : 'Individual buyer',
                ),
              ],
            ],
          ),
